...
exports.updateProfile=catchAsyncErrors(async(req,res,next)=>{
    ...
    const user= await User.findByIdAndUpdate(req.user.id,newUserData,{new:true,useFindAndModify:false,runValidators:true})
...
exports.getSingleUser=catchAsyncErrors(async(req,res,next)=>{
    ...
    if(!user)
    {
        return next(new ErrorHander(`User with the id ${req.params.id} not found`,400))
    }
...
exports.UpdateUserRole=catchAsyncErrors(async(req,res,next)=>{
    ...
    const user=await User.findByIdAndUpdate(req.params.id,newUserData,{new:true,useFindAndModify:false,runValidators:true})

    await user.save()
    res.status(201).json({success:true, message:"User upadated successfully"})
